...
export function useProbLab() {
  ...
  /**
   * 外れの消し方。"first" = 1 MOD 目 (他に何も付いていない): 外れは素の消去で確定 (外せる物が 1 つしか無い)、高貴も側のお告げ無しで
   * 打つ (どちらの側に付いても、外れなら素の消去)。オーナー 2026-09-25:「今回お告げ要らんよ、全部消去で消す動きで。1 MOD 目だよ」
   * "omen" = 他の MOD を守る: 反対側は固定済みで満杯、外れはその側の消去のお告げ、冒涜の外れは光
   */
  const missMode = ref<"first" | "omen">("first");
  ...
  function build(): void {
    ...
    // 開始: 1 MOD 目なら何も付いていない。それ以外は反対側が固定済みで満杯 (消えないし、そちらに付きもしない)。
    // この側は空 + 他の狙い (触らない) が others 個
    const other: Side = side === "prefix" ? "suffix" : "prefix";
    const otherFull = first ? [] : Array.from({ length: limits.value[other] }, () => ({ modId: null, side: other, fixed: true }));
    ...
    const otherPoolW = (floor: number): number => otherPool.reduce((s, id) => { const x = d.mods.get(id); return x ? s + w(x, 0, floor) : s; }, 0);
    ...
    for (const [orb, floor, label] of [["exalt_perfect", 50, "完全の高貴"], ["exalt_greater", 35, "上級の高貴"], ["exalt", 0, "普通の高貴"]] as const) {
      ...
      for (const v of variants) {
        ...
        // 1 MOD 目は側のお告げ無し: 反対側にも付く (両側に空き) ので、その側に付く分は半分。反対側に付いた物は素の消去で消す
        ...
        const nodes: SimNode[] = [
          ...
          { ...base, id: "e", action: { kind: "exalt", tier: orb, side: first ? null : side, catalyst: v.tag }, targets: target, onHit: "done", onMiss: "x" },
          { ...base, id: "x", action: { kind: "annul", side: first ? null : side }, onHit: "e", onMiss: "e" },
        ];
        out.push({
          key: `${orb}:${v.tag ?? ""}:${v.q}`, method: first ? `${label} (お告げ無し、外れは素の消去)` : `${label} + ${side === "prefix" ? "左側" : "右側"}のお告げ`,
    ...
    for (const [bone, floor, label] of [["desecrate", 0, "普通の骨"], ["desecrate_ancient", 40, "古代の骨"]] as const) {
      ...
      for (const rr of ["light", "overwrite"] as const) {
        ...
        const nodes: SimNode[] = rr === "light" ? [
          ...
          first ? { ...base, id: "l", action: { kind: "annul", side: null }, onHit: "d", onMiss: "d" } : { ...base, id: "l", action: { kind: "light" }, onHit: "d", onMiss: "d" },
        ] : [
        ...
        out.push({ key: `${bone}:${rr}`, method: `冒涜 (${label}、反響あり) + ${rr === "light" ? (first ? "外れは素の消去" : "光 + 消去") : "天体で上書き"}`, detail: `1 候補 ${(p1 * 100).toFixed(2)}%、6 候補で`, p: pHit, perTry, perMiss, safe: true, expected: perTry * tries + perMiss * (tries - 1), nodes, start: startOf({}) });
      }
    }
  ...
  watch([mod, minTier, itemLevel, othersOnSide, redoOthersDivine, prices, catalystQuality, missMode], () => build());
  ...
  return { data, prices, error, loading, baseName, itemLevel, modId, minTier, othersOnSide, redoOthersDivine, priceLabel, runs, pinned, usedPrices, catalystQuality, missMode, bases, cls, limits, mods, mod, tiers, rows, running, ensure, build, runAll, pin, unpin, jaOfBase, money };
}

 ...
 * (下の `FRACTURE_DECOY_NOTE`)。
 */
import { selfFracture } from "./self-fracture";
// 型は 2026-09-26 に tree-decide-types.ts へ分けた
import type { Candidate, DecidePrices, TreeListing } from "./tree-decide-types";
export type { Candidate, DecidePrices, TreeListing } from "./tree-decide-types";

/**
 * 固定の抽選から外れる MOD (「当て馬」)。オーナーの説明 (2026-09-23):「4 MOD の奴をフラクチャーテクで 3 MOD + 冒涜で
 * 4 MOD に見せて、フラクチャーを 3 分の 1 の確率にするのが主流」。**数合わせに乗せた冒涜の MOD は固定の抽選に入らない**。
 * ゲームのデータに裏づけは無く、**オーナーの実使用が根拠**。画面で断ること。
 */

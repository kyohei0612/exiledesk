 ...
 * どちらも**使い切ってから消せる**ので、完成品に MOD が残らないことがあります。
 *
 ...
 * どちらも「最大品質を 40% にする」なので、ここが**上限そのもの**。
 */
export const RAISED_MAX_QUALITY = 40;
/**
 * **インフューザーで超過できる幅 (%)**。
 *
 * オーナーの読み 2026-09-23:「多分神殿のインフューザーじゃね」── 当たり。クライアントの効果文:
 *   ヴァールカタリストのインフューザー
 *   「指輪またはアミュレットの品質を向上させる。**最大品質を最大 10% まで超過することができる**が、
 *     一定確率でコラプト化してしまう」
 * 武器 / 防具にも同じ物があります (ヴァール鍛冶 / 秘術 / 防具職人のインフューザー)。
 *
 * **代償がある**ので、超過ぶんは「狙って作る物」ではありません。コラプトすると
 * **もうクラフトできなくなります**。
 */
export const INFUSER_OVER_QUALITY = 10;
/** 実際に見かけうる最大品質 (%)。40 + インフューザーの超過 10 */
export const ABSOLUTE_MAX_QUALITY = RAISED_MAX_QUALITY + INFUSER_OVER_QUALITY;

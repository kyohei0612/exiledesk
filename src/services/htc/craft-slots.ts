 ...
 *
 * ## ここを見ずにエッセンス候補を足すと壊れます (2026-09-22 に実測)
 * 猛攻 (パーフェクトエッセンス) + アタック速度 の 2 目標は **45 神**で解けます。ところが
 * アタック速度に「エッセンスでもいい」を足すと ([[essence-route.ts]])、ソルバが
 * **クラフト MOD 2 個と数えて拒否**します ── 枠の候補として足しただけで、実際は片方しか
 * 使わないのに。だから `withEssenceAlternatives` は**この残り枠の範囲でしか足しません**。
 */

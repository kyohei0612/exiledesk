 ...
 *
 * 2026-09-23 追記: 上の実測はキャストスピードの重みが**仮置きの 1** だった時の物で、Craft of Exile の
 * 推定値で埋めた今はキャスピ 1 個なら数手で終わる ([[weight-overrides.ts]])。**低すぎる上限で分位点が
 * 嘘になる、という中身は変わらない**ので、下限 1,000 手はそのまま残す。
 */

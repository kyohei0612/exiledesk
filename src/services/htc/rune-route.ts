 ...
 * そして**外してもアイテムは得た物を保ちます**。だから「差す → その枠で作る → 外す」が成立し、
 * ソケットが 1 つ空いた完成品になります。
 *
 * ## これは実測ではなく裁定です
...
export type RuneUse =
  ...
  | "keep"
  /** 作るあいだだけ差して、最後に外す (ソケットが空く。「外しても残る」裁定に乗る) */
  | "pull";
...
export interface RuneRoute {
  ...
  use: RuneUse;
  reason: RuneReason;
  /** 完成品にルーンが残るか。残るなら値段にルーン代が乗り、買い手の条件も増える */
  keepsRune: boolean;
  /** このベースでの枠 (ルーンを織り込んだ後) */
  limits: ItemLimits;
...
/** 「外しても残る」裁定。根拠が 2 件の報告だけなので、乗る道筋には必ず付ける */
const PULL_CAVEAT =
  "ルーンを外してもアイテムは得た物 (増えた枠 / 引けた MOD) を保つ、という前提です。実使用の報告が根拠で、ゲームのデータには裏づけがありません。";
 ...
 *
 * **外すほうが常に得**です。完成品の中身は同じで、ソケットが 1 つ空く分だけ買い手に有利になる。
 * ルーン代はどちらでもかかります (装着した時点で払っている)。だから `pull` が出せる時は `pull`。

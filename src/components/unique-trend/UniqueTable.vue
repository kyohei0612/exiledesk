<!--
  ...
  1 行 = 1 ユニーク。日本語名を大きく、英名とベースを小さく。見出しを押すと並び替え、行を押すと詳細が開く。
-->
...
defineProps<{ rows: UniqueRow[]; trends: Map<number, UniqueTrend>; loadingTrends: boolean; openId: number | null }>();
const sortKey = defineModel<SortKey>("sortKey", { required: true });
const emit = defineEmits<{ toggle: [id: number] }>();

const money = (ex: number) => displayCurrency.money(ex, { ladder: "top" });
/** 取引所をブラウザで開く (設定の開く先 = 日本語サイト。即時購入・コラプト無し) */
async function openTrade(nameEn: string): Promise<void> {
  try { await openTrade2ForUnique({ nameEn, league: (marketStore.league.value?.Value ?? "").toLowerCase().replace(/\s+/g, "-"), noCorrupted: true }); } catch { /* 開けない環境 (ブラウザ開発) は何もしない */ }
}
      ...
      <thead class="bg-[var(--exile-color-bg-surface)] text-xs tracking-wider text-[var(--exile-color-text-secondary)]">
        <tr>
          ...
          <th :class="[th, sortable, sortKey === 'rise' || sortKey === 'fall' ? on : '']" class="text-right w-48" @click="clickChange">
            ...
            <span v-if="loadingTrends" class="ml-1 text-[10px] text-[var(--exile-color-text-tertiary)]">読込中…</span>
          </th>
          ...
          >
            ...
            <td class="px-3 py-2.5">
              <div class="flex items-center gap-3 min-w-0">
                ...
                <div class="min-w-0">
                  <div class="text-[var(--exile-color-text-primary)] truncate">{{ r.nameJa }}</div>
            ...
            <td class="px-3 py-2.5 text-right whitespace-nowrap tabular-nums text-sm text-[var(--exile-color-text-secondary)]">
              {{ trends.get(r.itemId)?.qty ?? "—" }}
            </td>
            <td class="px-3 py-2.5">
            ...
            <!-- 取引所へそのまま (即時購入・コラプト無し。API は使わず ?q= の URL を開く。オーナー 2026-09-26) -->
            <td class="px-3 py-2.5 text-right">
              <button type="button" class="rounded border border-[var(--exile-color-border-subtle)] px-2 py-0.5 text-xs hover:border-[var(--exile-color-accent-focus)] hover:text-[var(--exile-color-accent-focus)]"
                title="取引所 (即時購入・コラプト無し) をブラウザで開く" @click.stop="openTrade(r.nameEn)">開く ↗</button>
            </td>
          </tr>

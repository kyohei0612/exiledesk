 ...
 *   接頭辞 / 接尾辞 → **プレフィックス / サフィックス**。
 *   **ゲームは両方使っています。**クライアントの日本語表を全部数えると:
 *     カタカナ … PassiveSkills 9 件 + カレンシー / お告げの効果文 10〜12 件
 *     漢字     … ActiveSkills のスキル説明 1 件 + Mods の名前 2 件 (「接頭辞の」「接尾辞付き」)
 *   **クラフトの文脈 (カレンシー・お告げ・パッシブ) はカタカナ**なので、こちらに揃える。
 *   お告げの効果文が「次に使用する消去のオーブはサフィックスモッドをのみを取り除く」。
 *
 *   触媒 → **カタリスト**。ただし「触媒の高貴のお告げ」(Omen of Catalysing Exaltation) は
 *   公式にこの字なので、そこだけは触らない。
 *
 * ## corpus に入れていない物
 * `data-cache/client-export/tables/Japanese/` の生の表は**入れていません** (リポジトリに
 * 無い環境がある)。上の件数はそれを直接数えた結果で、`src/i18n/` の抽出物だけでは
 * ActiveSkills / Mods の名前は見えません。**「corpus に無い = 公式に無い」ではない**ことに注意。
 *
...
const BANNED = [
  ...
  {
    bad: "触媒", good: "カタリスト", why: "アイテムは「肉体のカタリスト」等",
    // 「触媒の高貴のお告げ」は公式名。オーナーの発言の引用は書き換えない (言った通りを残す)
    allow: ["触媒の高貴のお告げ", "触媒の品質、生贄の高貴", "品質上げた際の触媒成功率"],
  },
];

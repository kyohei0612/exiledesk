...
async fn main() {
    ...
    let search = trade2_search_with(None, SearchRequest { patient: false, league: league.clone(), query, site: None })
        .await
        .expect("search");
    ...
    let fetched = trade2_fetch_with(None, FetchRequest { patient: false, ids, query_id: qid, site: None }).await.expect("fetch");

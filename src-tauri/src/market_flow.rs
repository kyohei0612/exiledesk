...
/// 自動巡回で 1 巡にかける時間 (オーナー指示 2026-09-18: 20 分 → 「15 分はどーやろ」)。
///
/// 54 銘柄 = 108 リクエストを 15 分 → 8 秒に 1 回。search と fetch は別の枠なので
/// 各エンドポイントは 16 秒に 1 回になり、一番きつい 5 分窓 (30 回) でも 18 回で収まる
/// (余裕 1.7 倍。手動の再取得が挟まっても当たらない)。
/// これ以上詰めると 12 分で 1.3 倍、10 分で 1.1 倍になり、手動の取得とぶつかった時に 429 を踏む。
/// 周期がこれより短い時は周期に合わせる (1 巡が次の巡に食い込まないように)。
const SWEEP_TARGET_SECS: i64 = 15 * 60;
...
mod tests {
    ...
    fn spread_pace_finishes_a_sweep_in_the_target_window() {
        ...
        assert_eq!(pace, 8, "108 リクエスト × 8 秒 = 約 15 分");
        // 各エンドポイント (search / fetch) は 16 秒に 1 回 → 5 分窓で 18 回 (上限 30)
        assert!(300 / (pace * 2) <= 30);
        // 周期が短い時はそちらに合わせる (1 巡が次の巡に食い込まない)
        assert_eq!(spread_pace_secs(54, 600), 5);
        // 銘柄が 1 つなら 2 リクエストしかないので間隔は長くなる
        assert_eq!(spread_pace_secs(1, 2 * 3600), 450);
    }
